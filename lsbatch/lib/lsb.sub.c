...
#define SET_PARM_NUMBER(name, field, delnum, defnum) { \
    ...
    if(!esub_method) {
        char conf_path[1024];
        char* envdir = getenv("LSF_ENVDIR");
        if (envdir) {
            snprintf(conf_path, sizeof(conf_path), "%s/lsf.conf", envdir);
        } else {
            snprintf(conf_path, sizeof(conf_path), "/etc/lsf.conf");
        }
        char buf[1024];
